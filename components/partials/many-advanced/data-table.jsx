...
export function DataTable({
  ...
  // Sync rowSelection if selectedRowIds changes from parent
  useEffect(() => {
    ...
    if (selectedRowIds && Array.isArray(selectedRowIds)) {
      ...
      dataList.forEach((row) => {
        if (selectedRowIds.includes(row.id)) newSelection[row.id] = true;
      });
      // Robust deep compare: check both keys and values
      const keys1 = Object.keys(newSelection);
      const keys2 = Object.keys(rowSelection);
      const isSame =
        keys1.length === keys2.length &&
        keys1.every((k) => rowSelection[k] === true) &&
        keys2.every((k) => newSelection[k] === true);
      if (!isSame) {
        isSyncingFromParent.current = true;
        setRowSelection(newSelection);
      }
    }
  }, [selectedRowIds, dataList]);
