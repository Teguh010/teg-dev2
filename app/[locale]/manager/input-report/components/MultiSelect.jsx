...
const MultiSelect = ({
  ...
  const valueStrings = useMemo(() => value.map(v => String(v)), [value])
  ...
  }, [value, valueStrings, options, onChange])

  const handleRemove = useCallback((valueToRemove) => {
    ...
    <div>
      ...
      <div className="relative min-w-[250px]">
        <button
          ...
          disabled={disabled}
          className="w-full px-2 py-1 border rounded-md min-h-[30px] flex items-center justify-between text-left bg-white"
        >
          <div className="flex-1 flex flex-wrap gap-1 py-[0.7px]">
            {value.length > 0 ? (
        ...
        {open && (
          <div className="absolute z-50 w-full mt-1 border rounded-md shadow-lg max-h-[300px] overflow-auto bg-white">
            <div className="p-2">
              <input
                type="text"
