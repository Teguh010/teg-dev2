...
import withDevAuth from '@/hooks/withDevAuth';
...
const DeveloperMainLayout = ({ children }) => {
  const [open, setOpen] = useState(false);
  const location = usePathname();
  const mounted = useMounted();
  const { isMaintenanceMode } = useMaintenance();

  // Skip layout for developer login page
  if (location.includes('/developer/login')) {
    return <>{children}</>;
  }
...
export default withDevAuth(DeveloperMainLayout);
