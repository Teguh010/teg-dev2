...
export const controller = () => {
  ...
  const [selectedTranslationGroup, setSelectedTranslationGroup] = useState<number>(1); // Default to group 1 (Client Web)
  ...
  useEffect(() => {
    const fetchTranslations = async () => {
      setIsLoading(true);
      try {
        const result = await translationList(user?.token || null, currentLanguage, selectedTranslationGroup);
    ...
    if (user?.token && user?.username) {
      const shouldFetch = 
        ...
        // Translation group changed
        prevGroupRef.current !== selectedTranslationGroup;
  ...
  }, [user?.token, user?.username, currentLanguage, selectedTranslationGroup]);
  ...
  const refetchTranslations = async () => {
    ...
    try {
      ...
      const result = await translationList(latestUser?.token || null, currentLanguage, selectedTranslationGroup);
  ...
  const updateTranslation = async (translations: Translation | Translation[]) => {
    try {
      ...
      // Get the latest token from UserContext
      const latestUser = UserContext.operations.getUserRef();
      const result = await translationSetBulk(latestUser?.token || null, currentLanguage, formattedTranslations, selectedTranslationGroup);
  ...
  const addTranslations = async () => {
    ...
    try {
      // Get the latest token from UserContext
      const latestUser = UserContext.operations.getUserRef();
      const result = await translationSetBulk(latestUser?.token || null, currentLanguage, newTranslations, selectedTranslationGroup);
  ...
  const submitUploadedTranslations = async () => {
    ...
    try {
      // Get the latest token from UserContext
      const latestUser = UserContext.operations.getUserRef();
      const result = await translationSetBulk(latestUser?.token || null, currentLanguage, uploadedTranslations, selectedTranslationGroup);
