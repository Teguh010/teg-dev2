...
export const controller = () => {
  ...
  // Persist selected vehicle across token refreshes
  const [vehicle, setVehicleState] = useState(() => {
    if (typeof window === "undefined") return null;
    try {
      const key = "selected-vehicle";
  ...
  const [minMoving, setMinMoving] = useState(1);
  const [minStationary, setMinStationary] = useState(1);
  const [tripMode, setTripMode] = useState("");
  const [showStationaryIgnition, setShowStationaryIgnition] = useState(false);
  ...
  const defaultReportType = t("general.trip_and_stop");
 ...
 const [ignorePreList] = useState([
  ...
  const ignoreList = useMemo(() => {
    ...
    if (reportType === t("general.trip_only")) {
      // Don't ignore address for trip_only - we want to show it (formatted as from -> to)
      // Don't ignore from and to either - we need them to create the address format
      ...
      // Don't ignore address for trip_and_stop - we want to show it
      baseList.push({ title: t("general.next_address") });
    }
    return baseList;
  }, [ignorePreList, isGenerate, reportType]);
  ...
  const [orderListData] = useState([
    ...
    { title: "general.lat_lon" },
    { title: "general.avg_speed" },
    { title: "general.to" },
    { title: `general.avg_speed_(km)` },
    { title: "general.from" },
    { title: "general.next_address" },
    { title: "general.next_lat_lon" },
  ...
  // Keep selected vehicle persisted per user context
  useEffect(() => {
  ...
  const setVehicle = (val) => {
    ...
    if (typeof window !== "undefined") {
      try {
        localStorage.setItem("selected-vehicle", JSON.stringify(val));
      } catch {}
    }
  };

  // Function to fetch trajectory data
  const fetchTrajectoryData = async () => {
    
    // If we already have trajectory data, don't fetch again
    if (trajectoryData.length > 0) {
      return;
    }
    
    if (!user?.token || !vehicle || !startDate || !endDate) {
      return;
    }
    ...
    try {
      ...
      const data = await objectTrajectoryNormalized(user.token, vehicle, startDate, endDate);
  ...
  const filterData = (objects: any[]) => {
    return objects.map((obj) => {
      ...
      // Handle address column based on report type
      if (reportType === t("general.trip_and_stop")) {
        // For trip and stop: set address to null for moving data
        if (newObj.state === "moving") {
          newObj.address = null;
        } else {
          // For stationary data, use the address field or from field
          newObj.address = newObj.address || newObj.from;
        }
      } else if (reportType === t("general.trip_only")) {
        // For trip only: format address as from -> to
        if (newObj.from && newObj.to) {
          newObj.address = `${newObj.from} → ${newObj.to}`;
        }
      } else if (reportType === t("general.stop_only")) {
        // For stop only: use the address field or from field
        newObj.address = newObj.address || newObj.from;
      }
      ...
      // Combine lat and lon into one column (but keep original fields for map interaction)
      if (newObj.lat !== undefined && newObj.lon !== undefined) {
        newObj.lat_lon = `${newObj.lat}, ${newObj.lon}`;
        // Don't delete lat and lon - keep them for map interaction
      }

      // Combine next_lat and next_lon into one column (but keep original fields for map interaction)
      if (newObj.next_lat !== undefined && newObj.next_lon !== undefined) {
        newObj.next_lat_lon = `${newObj.next_lat}, ${newObj.next_lon}`;
        // Don't delete next_lat and next_lon - keep them for map interaction
      }
  ...
  // Calculate initial column visibility based on report type only (not dependent on data)
  const initialColumnVisibility = useMemo(() => {
    const baseVisibility = {
      ...
      lon: false,
      address: true,
      from: false, // Hide 'from' column by default
      lat_lon: false, // Show combined lat_lon column by default
      next_lat_lon: false, // Show combined next_lat_lon column by default
      avg_speed: false, // Hide avg_speed column by default
      time_from: true, // Show time_from column (will display both times)
      time_to: false, // Hide time_to column since it's shown in time_from
      id: false
    };

    const isTripAndStop = reportType === t("general.trip_and_stop");
    const isTripOnly = reportType === t("general.trip_only");
    
    // For trip only reports, show 'address' (formatted as from -> to) but hide 'from' and 'to' columns
    // For trip and stop reports, show 'address' but hide 'from' and 'to' columns
    if (isTripOnly) {
      baseVisibility.from = false; // Hide 'from' column for trip only
      baseVisibility.to = false; // Hide 'to' column for trip only
      baseVisibility.address = true; // Show 'address' column for trip only (formatted as from -> to)
    } else if (isTripAndStop) {
      baseVisibility.from = false; // Hide 'from' column for trip and stop
      baseVisibility.to = false; // Hide 'to' column for trip and stop
      baseVisibility.address = true; // Show 'address' column for trip and stop
    } else {
      // For stop only reports, show 'from' and 'to' columns but hide 'address'
      baseVisibility.from = true; // Show 'from' column for stop only
      baseVisibility.to = true; // Show 'to' column for stop only
      baseVisibility.address = false; // Hide 'address' column for stop only
    }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Clear trajectory data when regenerating
        setTrajectoryData([]);
        ...
        const params = {
          ...
          ...(tripMode !== null && tripMode !== undefined && tripMode !== "" && { trip_mode: Number(tripMode) }),
        ...
        // Initialize loading states for rows that need address fetching
        const initialLoadingStates = {};
        dataObjectTripStop.data.forEach((obj, index) => {
          // Check what address field is needed based on report type
          let needsAddressFetching = false;
          
          if (reportType === t("general.trip_and_stop")) {
            // For trip and stop: check if address field is null but has coordinates
            const needsFromAddress = obj.lat && obj.lon && !obj.address;
            const needsToAddress = obj.next_lat && obj.next_lon && !obj.next_address;
            needsAddressFetching = needsFromAddress || needsToAddress;
            
          } else if (reportType === t("general.stop_only")) {
            // For stop only: check from/to fields
            needsAddressFetching = (obj.lat && obj.lon && !obj.address) || (obj.next_lat && obj.next_lon && !obj.next_address);
          } else if (reportType === t("general.trip_only")) {
            // For trip only: check from/to fields
            needsAddressFetching = (obj.lat && obj.lon && !obj.address) || (obj.next_lat && obj.next_lon && !obj.next_address);
          }
          
          if (needsAddressFetching) {
            initialLoadingStates[index] = true;
          }
        });
        setAddressLoadingStates(initialLoadingStates);
        ...
        // Clear loading states after addresses are fetched
