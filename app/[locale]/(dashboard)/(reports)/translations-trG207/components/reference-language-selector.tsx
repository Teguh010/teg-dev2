...
interface ReferenceLanguageSelectorProps {
  languages: { name: string; slug: string; flag: any }[];
  selectedLanguage: string;
  onSelect: (slug: string) => void;
  onCheckMissingKeys: () => void;
  t: (key: string) => string;
}
...
const ReferenceLanguageSelector = ({
  ...
  onCheckMissingKeys,
  t
}: ReferenceLanguageSelectorProps) => {
  return (
    <div className="flex items-center gap-4">
      <label className="font-medium">{t('translation_page.reference_language')}:</label>
      <DropdownMenu>
      ...
      <Button
        onClick={onCheckMissingKeys}
        variant="outline"
      >
        {t('translation_page.check_missing_keys')}
      </Button>
    </div>
  );
};

export default ReferenceLanguageSelector;