...
const RawMessagesChart = ({
  ...
  // Cache the available fields to avoid recalculation
  const [cachedAvailableFields, setCachedAvailableFields] = useState<string[]>([]);

  useEffect(() => {
    onPointClickRef.current = onPointClick;
  }, [onPointClick]);

  // Cache available fields when data changes
  useEffect(() => {
    const firstItem = dataValidRawMessages[0];
    if (!firstItem) {
      setCachedAvailableFields([]);
      return;
    }
    ...
    setCachedAvailableFields(filteredFields);
  }, [dataValidRawMessages, selectedDataTypes, datatypeList]);

  // Fast filter - completely independent of data processing
  const availableFields = useMemo(() => {
    if (!showGpsspeed) {
      return cachedAvailableFields.filter((field) => field !== "gpsspeed");
    }
    return cachedAvailableFields;
  }, [cachedAvailableFields, showGpsspeed]);
