...
export const controller = () => {
  ...
  const [showGpsspeed, setShowGpsspeed] = useState<boolean>(true); // Default centang
  
  // Memoized setShowGpsspeed to prevent unnecessary re-renders
  const setShowGpsspeedMemo = useCallback((value: boolean) => {
    setShowGpsspeed(value);
  }, []);
    ...
    () => [
      {
        ...
        value: (val: unknown, rowData?: Record<string, unknown>) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Default data type IDs to send silently when no data types are selected
        const DEFAULT_DATA_TYPE_IDS = [300001, 300002];
        
        // Use selectedDataTypes if available, otherwise use ioIdsFilter, otherwise use default silently
        if (selectedDataTypes.length > 0) {
          ...
          // Silently use default data types
          params.io_ids_filter = DEFAULT_DATA_TYPE_IDS;
        }
  ...
  return {
    ...
    operations: {
      ...
      setNumberRows,
      setShowGpsspeed: setShowGpsspeedMemo,
    },
