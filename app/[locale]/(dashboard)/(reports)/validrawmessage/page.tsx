...
const ValidRawMessage = () => {
  ...
  const [isZoomEnabled, setIsZoomEnabled] = useState(true); // Zoom enable/disable state
  const resetZoomRef = useRef(null); // Ref untuk reset zoom function
  ...
  const handleMarkerClick = (id) => {
    setActiveRowId(id); // set active row untuk highlighting, tidak mengubah selectedRowIds
  };

  // Handler ketika row di table di klik
  const handleRowClick = (id) => {
    setActiveRowId(id); // set active row untuk highlighting dan zoom
    
    // Trigger zoom to marker if map is visible and zoom is enabled
    if (isMapVisible && mapComponentRef.current && isZoomEnabled) {
      const clickedData = dataForTable.find(item => item.id === id);
      if (clickedData && typeof clickedData.lat === 'number' && typeof clickedData.lon === 'number') {
        // Trigger zoom to marker by calling a method on the map component
        if (mapComponentRef.current.zoomToMarker) {
          mapComponentRef.current.zoomToMarker(clickedData.lat, clickedData.lon);
        }
      }
    }
  };
  ...
  // Handler untuk menerima reset function dari map
  ...
  const handleSelectedRowsChange = useCallback(
    (rows) => {
      ...
      if (
        ...
        // Tidak mengubah activeRowId saat checkbox selection
        // activeRowId hanya diubah saat row click atau marker click
      }
    },
                                  ...
                                  >
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                                      ...
                                      >
                                        {isZoomEnabled ? (
                                          <>
                                            <ToggleRight size={12} color="#10B981" />
                                            <span style={{ color: "#10B981" }}>Zoom On</span>
                                          </>
                                        ) : (
                                          <>
                                            <ToggleLeft size={12} color="#9CA3AF" />
                                            <span style={{ color: "#9CA3AF" }}>Zoom Off</span>
                                          </>
                                        )}
                                      </button>
          ...
          {models.isLoading || !models.dataValidRawMessages?.length ? (
            <div className='space-y-4'>
              ...
              <AdvancedTable
                ...
                onRowClick={handleRowClick} // <-- tambahkan prop baru
