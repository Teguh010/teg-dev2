...
export const controller = () => {
  ...
  const styleColumnList = useMemo(
    () => [
      {
        title: t("gpstime"),
        header: () => isDesktop ? "sticky left-0 top-0 bg-default-300" : "bg-default-300",
        value: (val: unknown, rowData?: Record<string, unknown>) => {
          const ignitionValue = rowData?.ignition || rowData?.["(input)_ignition"];
          const stickyClass = isDesktop ? "sticky left-0 z-10" : "";
          return `${stickyClass} ${ignitionValue ? "bg-green-100" : "bg-white"}`;
        },
      },
    ],
    [t, isDesktop]
